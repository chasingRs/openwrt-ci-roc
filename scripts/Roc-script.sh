# 固件名称 & 编译署名和时间
sed -i "s/hostname='.*'/hostname='PWRT'/g" package/base-files/files/bin/config_generate
sed -i "s#_('Firmware Version'), (L\.isObject(boardinfo\.release) ? boardinfo\.release\.description + ' / ' : '') + (luciversion || ''),# \
            ...
            E('span', {}, [\n \
                ...
                : '') + (luciversion || '') + ' / ',\n \
            E('a', {\n \
                href: 'https://github.com/chasingRs/openwrt-ci-roc/releases',\n \
                target: '_blank',\n \
                rel: 'noopener noreferrer'\n \
                }, [ 'Built by Pkjinfinity $(date "+%Y-%m-%d %H:%M:%S")' ])\n \
